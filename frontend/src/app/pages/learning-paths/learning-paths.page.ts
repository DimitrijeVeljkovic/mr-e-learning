...
export class LearningPathsPage implements ViewDidEnter {
  ...
  public handleSearch(event: any) {
    this.filteredPaths = this.learningPaths.filter((path) =>
      path.title.toUpperCase().includes(event.detail.value.toUpperCase())
    );
  }

  public startCourse(courseId: number) {
    this._courseService.startCourse({ courseId }).subscribe(
      (res) => {
  ...
  public bookmarkCourse(courseId: number) {
    this._courseService.bookmarkCourse({ courseId }).subscribe(
      (res) => {
