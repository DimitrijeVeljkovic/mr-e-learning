...
export class CourseService {
  ...
  public getBookmarkedCourses(): Observable<{ course: Course }[]> {
    return this._http.get(
      `${API_ROUTES.COURSES.BOOKMARK}?userId=${
        this._userService.getAuthData().userId
      }`
    ) as Observable<{ course: Course }[]>;
  }

  public getInProgressCourses(): Observable<InProgressCourse[]> {
    return this._http.get(
      `${API_ROUTES.COURSES.IN_PROGRESS}?userId=${
        this._userService.getAuthData().userId
      }`
    ) as Observable<InProgressCourse[]>;
  }

  public getSingleInProgressCourse(
    courseId: number
  ): Observable<InProgressCourse> {
    return this._http.get(
      `${API_ROUTES.COURSES.IN_PROGRESS}/${courseId}?userId=${
        this._userService.getAuthData().userId
      }`
    ) as Observable<InProgressCourse>;
  }
  ...
  public getCompletedCourses(): Observable<CompletedCourse[]> {
    return this._http.get(
      `${API_ROUTES.COURSES.FINISH}?userId=${
        this._userService.getAuthData().userId
      }`
    ) as Observable<CompletedCourse[]>;
  }

  public startCourse(body: {
    courseId: number;
  ...
  public getCounts(userId: string | null): Observable<{
    ...
    return this._http.get(
      `${API_ROUTES.COURSES.COUNT}${queryParams}`
    ) as Observable<{
      courseCount: number;
      learningPathCount: number;
      inProgressCount: number;
  ...
  public submitTest(
    ...
    return this._http.post(
      `${API_ROUTES.COURSES.SUBMIT}/${courseId}?userId=${
        this._userService.getAuthData().userId
      }`,
      body
    ) as Observable<{ message: string }>;
